...
links = []  
for i in range(1,16):
    url='https://www.transfermarkt.com.tr/wettbewerbe/europa?page='
    page = url+str(i)
    links.append(page)
##############################################################
...
for i in range(len(links)):
    tree = requests.get(links[i], headers = headers)
    soup = BeautifulSoup(tree.content, 'html.parser')
    e = soup.select("tr.odd, tr.even")
    for i in e:
        links1.append(i.select("a")[1]["href"])
for j in range(len(links1)):
       links1[j] = "https://www.transfermarkt.com.tr"+links1[j]

string = "/plus/?saison_id="
list2 = list(map(lambda orig_string: orig_string + string, links1))
######################################################################
new_links = []
...
print(datetime.now().time())
print('Veri Çerçevesi Oluşturuldu') #csv olarak yazdır.
